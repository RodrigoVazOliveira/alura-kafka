...
public class NewOrderMain {
    public static void main(String[] args) throws ExecutionException, InterruptedException {
        ...
        KafkaDispatcher<String> kafkaDispatcherEmail = new KafkaDispatcher<>();
        for (int i = 0; i < 10; i++) {
            Order order = new Order(UUID.randomUUID().toString(), UUID.randomUUID().toString(),
                    BigDecimal.valueOf(Math.random() * 5000 + 1));
            kafkaDispatcherEmail.sendMessage("ECOMMERCE_SEND_MAIL", UUID.randomUUID().toString(), "Thank you, new order processing success!");
